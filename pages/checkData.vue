...
const showRenameModal = ref(false); // ควบคุมการแสดง Modal เปลี่ยนชื่อ
const newFileName = ref(''); // ชื่อไฟล์ใหม่
const selectedFileToRename = ref(''); // ไฟล์ที่เลือกเพื่อเปลี่ยนชื่อ
...
// ฟังก์ชันเปิด Modal เปลี่ยนชื่อ
const openRenameModal = (fileName) => {
  selectedFileToRename.value = fileName;
  showRenameModal.value = true;
};

// ฟังก์ชันปิด Modal เปลี่ยนชื่อ
const closeRenameModal = () => {
  showRenameModal.value = false;
  selectedFileToRename.value = '';
  newFileName.value = '';
};
...
const renameFile = async () => {
  if (!newFileName.value) {
    await Swal.fire({
      title: 'กรุณากรอกชื่อไฟล์ใหม่',
      text: 'คุณต้องกรอกชื่อไฟล์ใหม่ก่อนดำเนินการต่อ',
      icon: 'warning',
  ...
  try {
    const response = await axios.post(`${config.apiServer}/api/rename-file`, {
      old_name: selectedFileToRename.value,
    ...
    if (response.data.success) {
      ...
      closeRenameModal();
      await fetchFiles(); // รีเฟรชรายการไฟล์หลังจากเปลี่ยนชื่อ
    }
  } catch (error) {
    await Swal.fire({
      title: 'เกิดข้อผิดพลาด!',
      text: error.response?.data?.detail || 'เกิดข้อผิดพลาดในการเปลี่ยนชื่อไฟล์',
      icon: 'error',
      confirmButtonText: 'ตกลง',
    });
...
const filteredRegisteredFiles = computed(() => {
  if (!searchQuery.value) {
    return registeredFiles.value;
  }
  return registeredFiles.value.filter(file =>
    file.user_name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
...
const paginatedRegisteredFiles = computed(() => {
  ...
  return filteredRegisteredFiles.value.slice(start, end);
});
...
const totalPagesRegistered = computed(() => {
  return Math.ceil(filteredRegisteredFiles.value.length / itemsPerPageRegistered.value);
});
...
watch(searchQuery, () => {
  currentPageRegistered.value = 1; // รีเซ็ตหน้าเป็น 1 เมื่อค้นหา
});
</script>
...
<template>
  <div class="p-8">
    ...
    <div class="flex justify-center mb-6">
      <button @click="showRegisteredFiles = false" :class="{
      ...
      <button @click="showRegisteredFiles = true" :class="{
        'bg-blue-500 text-white': showRegisteredFiles,
        'bg-gray-200 text-gray-700': !showRegisteredFiles,
    ...
    <!-- ช่องค้นหา (แสดงเฉพาะเมื่ออยู่ในโหมดไฟล์ที่ลงทะเบียนแล้ว) -->
    <div v-if="showRegisteredFiles" class="mb-6">
      <input v-model="searchQuery" type="text" placeholder="ค้นหาตามชื่อผู้ใช้หรือชื่อไฟล์"
        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
    </div>

    <!-- แสดงรายการไฟล์ PDF -->
    <div v-if="(showRegisteredFiles ? paginatedRegisteredFiles : paginatedMissingFiles).length > 0"
      ...
      <table class="min-w-full">
        ...
        <tbody class="divide-y divide-gray-200">
          <tr v-for="file in (showRegisteredFiles ? paginatedRegisteredFiles : paginatedMissingFiles)"
            :key="file.file_name || file" class="hover:bg-gray-50 transition-colors">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
              <button v-if="!showRegisteredFiles" @click="openRenameModal(file)"
                class="bg-yellow-500 text-white px-4 py-2 rounded-lg hover:bg-yellow-600 transition-colors">
                เปลี่ยนชื่อ
              </button>
              <button v-if="!showRegisteredFiles" @click="openRegisterModal(file)"
                class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition-colors">
                ลงทะเบียน
              </button>
              <button @click="deleteFile(file.file_name || file)"
                class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition-colors">
                ลบไฟล์
              </button>
            </td>
      ...
      <div class="flex justify-between items-center p-4 border-t border-gray-200">
        ...
        <span class="text-sm text-gray-700">
          หน้า {{ showRegisteredFiles ? currentPageRegistered : currentPageMissing }} จาก {{ showRegisteredFiles ?
            totalPagesRegistered : totalPagesMissing }}
        </span>
        <button
          @click="showRegisteredFiles ? goToPageRegistered(currentPageRegistered + 1) : goToPageMissing(currentPageMissing + 1)"
    ...
    <!-- Modal สำหรับเปลี่ยนชื่อไฟล์ -->
<div v-if="showRenameModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
  <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
    <h2 class="text-xl font-bold mb-4">เปลี่ยนชื่อไฟล์ {{ selectedFileToRename }}.pdf</h2>
    <form @submit.prevent="renameFile">
      <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700">ชื่อไฟล์ใหม่</label>
        <input
          v-model="newFileName"
          type="text"
          class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
